...
st.set_page_config(page_title="RQ6 — Technology EOL", page_icon="📊", layout="wide")
inject_css()
rq = rq_content(6)
st.title(rq["title"])
...
DATA_PATHS = [ROOT / "outputs" / "reports" / "incident_downtime_by_lifecycle.csv", ROOT / "data" / "processed" / "incident_downtime_by_lifecycle.csv"]
df = None
for p in DATA_PATHS:
    if p.exists():
        df = pd.read_csv(p)
        break
if df is None:
    st.error("Required data file was not found: incident_downtime_by_lifecycle.csv")
    st.stop()
...
with tabs[1]:
    ...
    xkey = "Lifecycle_Status"
    ykey = "downtime_per_asset"
