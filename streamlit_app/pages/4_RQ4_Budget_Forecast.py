...
st.set_page_config(page_title="RQ4 — Technology EOL", page_icon="📊", layout="wide")
inject_css()
rq = rq_content(4)
st.title(rq["title"])
...
DATA_PATHS = [ROOT / "outputs" / "reports" / "refresh_budget_planning_summary.csv", ROOT / "data" / "processed" / "refresh_budget_planning_summary.csv"]
df = None
for p in DATA_PATHS:
    if p.exists():
        df = pd.read_csv(p)
        break
if df is None:
    st.error("Required data file was not found: refresh_budget_planning_summary.csv")
    st.stop()
...
with tabs[1]:
    ...
    xkey = "EOL_Year"
    ykey = "estimated_replacement_cost"
