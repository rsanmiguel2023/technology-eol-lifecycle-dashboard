...
st.set_page_config(page_title="RQ1 — Technology EOL", page_icon="📊", layout="wide")
inject_css()
rq = rq_content(1)
st.title(rq["title"])
...
DATA_PATHS = [ROOT / "outputs" / "reports" / "asset_risk_model.csv", ROOT / "data" / "processed" / "asset_risk_model.csv"]
df = None
for p in DATA_PATHS:
    if p.exists():
        df = pd.read_csv(p)
        break
if df is None:
    st.error("Required data file was not found: asset_risk_model.csv")
    st.stop()
