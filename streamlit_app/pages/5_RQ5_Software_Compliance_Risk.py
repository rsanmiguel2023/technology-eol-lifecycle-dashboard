...
st.set_page_config(page_title="RQ5 — Technology EOL", page_icon="📊", layout="wide")
inject_css()
rq = rq_content(5)
st.title(rq["title"])
...
DATA_PATHS = [ROOT / "outputs" / "reports" / "software_compliance_risk_summary.csv", ROOT / "data" / "processed" / "software_compliance_risk_summary.csv"]
df = None
for p in DATA_PATHS:
    if p.exists():
        df = pd.read_csv(p)
        break
if df is None:
    st.error("Required data file was not found: software_compliance_risk_summary.csv")
    st.stop()
